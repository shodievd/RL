...
# For creating MLP
def mlp(sizes, activation=nn.ReLU, output_activation=nn.Identity, squeeze_output=False) -> nn.Module:
    n = len(sizes)
    layers = []
...
# P_avg: w <- w * p + (1 - p) * w_t
def polyak_avg(target: nn.Module, source: nn.Module, polyak: float):
